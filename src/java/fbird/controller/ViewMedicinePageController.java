 ...
 * @author Louis Kien
 */
public class ViewMedicinePageController extends HttpServlet {

    private static final String ERROR = "MainController?action=";
    private static final String SUCCESS = "medicine.jsp";
        ...
        try {
            ...
            if (listProduct.size() > 0) {
                ...
                for (ProductDTO list : listProduct) {
                    if (list.getCategoryID() == 4) {
                        listFood.add(list);
                    }
                }
                if (listFood.size() > 0) {
                    request.setAttribute("LIST_MEDICINE_PAGE", listFood);
                    url = SUCCESS;
                }
            }
        } catch (Exception e) {
